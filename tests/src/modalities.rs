...
fn should_toggle_events_mode() {
    ...
    let set_modalities_call =
        cowl_swap_set_modalities(&mut builder, &cowl_swap_contract_hash, &owner, None);
    ...
    let actual_event: SetModalities = get_event(&builder, &cowl_swap_contract_hash.into(), 0);
    assert_eq!(
    ...
    let dictionary_seed_uref = *builder
        .query(None, cowl_swap_contract_hash.into(), &[])
